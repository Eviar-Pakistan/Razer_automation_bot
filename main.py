...
def setup_inputs():
    ...
    global entry_link, entry_email, entry_password
    entry_link = tk.Entry(input_frame, width=50)
    entry_link.pack()
    credentials = load_credentials()
    has_saved_account = bool(credentials)
    if not has_saved_account:
        ...
        saved_email = next(iter(credentials))
        tk.Label(input_frame, text=f"Using saved account: {saved_email}", fg="blue").pack()
    def on_submit():
        global global_email, global_password, global_link
...
def save_voucher(email, product, code, serial, url=None):
    try:
        with open("vouchers.txt", "a") as f:
            line = f"{email} | {product} | Code: {code} | Serial: {serial}"
            if url:
                line += f" | URL: {url}"
            f.write(line + "\n")
        print("📄 Voucher saved to vouchers.txt")
    except Exception as e:
...
def handle_product_click(product_name, quantity):
    ...
    try:
        ...
        for i in range(quantity):
            ...
            print(f"🌐 Navigated to transaction page: {transaction_url}")

            # Wait for PIN code and serial to be available
            page.wait_for_selector(".pin-code")
            page.wait_for_function("document.querySelector('.pin-code')?.textContent.trim().length > 0")
            page.wait_for_function("document.querySelector('.pin-serial-number')?.textContent.trim().length > 0")

            voucher_code = page.locator(".pin-code").text_content().strip()
            serial_number = page.locator(".pin-serial-number").text_content().strip().replace("S/N:", "").strip()

            save_voucher(global_email, product_name, voucher_code, serial_number, url=transaction_url)
            print(f"💾 Voucher saved: {voucher_code} | Serial: {serial_number}")

            if i < quantity - 1:
                page.goto(f"{global_link}")
                page.wait_for_selector(".selection-tile__text")
                page.click(f"text={product_name}")
                page.click(".selection-tile-promos__details")
